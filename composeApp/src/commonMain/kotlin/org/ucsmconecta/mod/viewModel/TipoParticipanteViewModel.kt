...
class TipoParticipanteViewModel(
    private val repository: ColaboradorRepository
): ViewModel() {
    private val _uiState = MutableStateFlow<UiState<List<DataTipoParticipanteResponse>>>(UiState.Idle)
    val uiState: StateFlow<UiState<List<DataTipoParticipanteResponse>>> = _uiState

    fun cargarDatosTipoParticipantes() {
        _uiState.value = UiState.Loading

        viewModelScope.launch {
            val result = repository.obtenerTodosLosTiposParticipantes()
            result.fold(
                onSuccess = { response ->
                    if (response.isEmpty()) {
